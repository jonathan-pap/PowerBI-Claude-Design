...
Visual type: `pivotTable` (matrix) or `tableEx` (flat table)
...
Visual type: `clusteredColumnChart` (vertical) or `clusteredBarChart` (horizontal)
...
Every Power BI visual has an exact `visualType` string. Using the wrong name
(e.g. `columnChart` instead of `clusteredColumnChart`) will cause errors or
unexpected behaviour. Always use the values from this table.

| `visualType` | Display name | Notes |
|--------------|-------------|-------|
| `clusteredBarChart` | Bar chart (horizontal) | Not `barChart` |
| `clusteredColumnChart` | Column chart (vertical) | Not `columnChart` |
...
| `tableEx` | Table | Not `table` |
| `pivotTable` | Matrix | Not `matrix` |
| `slicer` | Slicer | |
| `treemap` | Treemap | |
